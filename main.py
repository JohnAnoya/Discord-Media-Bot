...
port_url = "http://localhost:8191/v1"
...
async def on_ready(): 
    print("Bot is Online!")
    await client.add_cog(music_cog(client)), await client.add_cog(game_server_cog(client))
...
@client.tree.command(name = "mediahelp", description="bot commands for downloading media onto the jellyfin server")
async def media_help(interaction: discord.Interaction): 
...
@client.tree.command(name = "ythelp", description="bot commands for playing YouTube music")
...
@client.tree.command(name = "jellyfin", description="info for jellyfin (URL + login credentials)")
async def jellyfin(interaction: discord.Interaction): 
...
@client.tree.command(name = "search", description="search for media (movies, tv, anime)")
...
async def search(interaction: discord.Interaction, media_name:str, media_type: discord.app_commands.Choice[str]): 
   await interaction.response.send_message("Searching For " + media_name)
  
   if media_type.value == "tv" or media_type.value == "anime" : 
      await sonarr_organization(interaction, media_name)
      return
   
   elif media_type.value == "movies": 
      await radarr_organization(interaction, media_name)
      return

@client.tree.command(name = "displaydownload", description="show torrent currently downloading (using a Hash ID)")
@app_commands.describe(hash_id = "What is the hash ID of the torrent?")
async def display_download(interaction: discord.Interaction, hash_id:str):
...
@client.tree.command(name = "displayalldownloads", description="show all torrents currently downloading")
async def display_all_downloads(interaction: discord.Interaction):
...
@client.tree.command(name = "listlibrary", description="Lists all media library shows/movies and their series ID")
...
async def list_library(interaction: discord.Interaction, media_type: discord.app_commands.Choice[str]): 
...
@client.tree.command(name = "deletefromlibrary", description="Delete a show or movie from the library (using the series ID)")
@app_commands.describe( media_type = "Which type of media would you like to remove from the library? (Show or Movie)")
@app_commands.choices(media_type = [
   discord.app_commands.Choice(name = "Movies", value = "movie"),
   discord.app_commands.Choice(name = "TV Shows/Anime", value = "show")
])
async def delete_from_library(interaction: discord.Interaction, media_type: discord.app_commands.Choice[str], series_id: str):   
...
@client.tree.command(name = "pause", description="pause torrent currently downloading (using a Hash ID)")
@app_commands.describe(hash_id = "What is the hash ID of the torrent?")
async def pause(interaction: discord.Interaction, hash_id:str):
...
@client.tree.command(name = "pauseall", description="pause all torrents currently downloading")
...
@client.tree.command(name = "resume", description="resume torrent currently downloading (using a Hash ID)")
@app_commands.describe(hash_id = "What is the hash ID of the torrent?") 
async def resume(interaction: discord.Interaction, hash_id:str):
...
@client.tree.command(name = "resumeall", description="resume all torrents currently downloading")
...
@client.tree.command(name = "deletedownload", description="delete torrent currently downloading (using a Hash ID)")
@app_commands.describe(hash_id = "What is the hash ID of the torrent?") 
async def delete_download(interaction: discord.Interaction, hash_id:str):
...
@client.tree.command(name = "deletealldownloads", description="delete all torrents currently downloading")
async def delete_all_downloads(interaction: discord.Interaction):
...
@client.tree.command(name = "osmows", description="someone just ate osmows")
@app_commands.describe(osmows_eater = "What is the name of the osmows eater?") 
async def osmows(interaction: discord.Interaction, osmows_eater:str):
  print(len(osmows_images))
  rand_img = random.randint(0, len(osmows_images) - 1)

  osmows_embed = discord.Embed(title= "🚨OSMOWS EMERGENCY ALERT🚨", color= 15548997, description= osmows_eater + " ate osmows gg")
  osmows_embed.set_image(url=osmows_images[rand_img])
